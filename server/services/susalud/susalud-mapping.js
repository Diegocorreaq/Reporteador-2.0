...
export const LEGACY_FORMULAS_BY_BLOCK = {
  UCI: {
    TOTAL: 'SUM(MAX(camas/CAMASARQ) por servicio)',
    INOPERATIVOS: 'SUM(MAX(camas/CAMASARQ) - MIN(SUM(chabi), MAX(camas/CAMASARQ)) por servicio)',
    OPERATIVOS: 'SUM(MIN(SUM(chabi), MAX(camas/CAMASARQ)) por servicio)',
    LIBRES: 'SUM(MIN(SUM(clibr), operativos - ocupados) por servicio)',
    OCUPADOS: 'SUM(MIN(MAX(SUM(cocup), MAX(tocupa)), operativos) por servicio)',
    SIN_VM: 'ocupados - SUM(c_vm oficial)',
    CON_VM: 'SUM(c_vm)',
  ...
  UCIN: {
    ...
    LIBRES: 'SUM(MIN(SUM(clibr), operativos - ocupados) por servicio)',
    OCUPADOS: 'SUM(MIN(MAX(SUM(cocup), MAX(tocupa)), operativos) por servicio)',
    S_OXIGENO: 'ocupados - (SUM(c_vm oficial) + SUM(c_oxi oficial))',
    C_OXIGENO: 'SUM(c_oxi)',
    CON_VM: 'SUM(c_vm)',
    RESERVA: '0 (legacy: no suma ctran en camas_susalud)',
  },
  HOSPITALIZACION: {
    ...
    LIBRES: 'SUM(MIN(SUM(clibr), operativos - ocupados) por servicio)',
    OCUPADOS: 'SUM(MIN(MAX(SUM(cocup), MAX(tocupa)), operativos) por servicio)',
    S_OXIGENO: 'ocupados - SUM(c_oxi oficial)',
    C_OXIGENO: 'SUM(c_oxi)',
    RESERVA: '0 (legacy: no suma ctran en camas_susalud)',
  },
  EMERGENCIA: {
    ...
    LIBRES: 'SUM(MIN(SUM(clibr), operativos - ocupados) por servicio)',
    OCUPADOS: 'SUM(MIN(MAX(SUM(cocup), MAX(tocupa)), operativos) por servicio)',
    S_OXIGENO: 'ocupados - SUM(c_vm oficial)',
    C_OXIGENO: 'SUM(c_oxi)',
    CON_VM: 'SUM(c_vm)',
    RESERVA: '0 (legacy: no suma ctran en camas_susalud)',
  },
